...
describe("Poll CRUD Integration Tests", () => {
  ...
  it("fetches polls with pagination metadata", async () => {
    await signupAndGetCookie("list@example.com");
    await Poll.create({
      title: "Public Poll",
  ...
  it("allows owner to delete their poll", async () => {
    ...
    const res = await request(app).delete(`/api/polls/${poll.pollCode}`).set("Cookie", cookie);
  ...
  it("prevents non-owners from deleting polls", async () => {
    ...
    const res = await request(app).delete(`/api/polls/${poll.pollCode}`).set("Cookie", thiefCookie);
  ...
  it("allows owner to close their poll", async () => {
    ...
    const res = await request(app).patch(`/api/polls/${poll.pollCode}/close`).set("Cookie", cookie);
