...
import { Plus, X, Loader2, Target, Globe, Clock, Zap, MessageSquare, ListTodo, Shield, CheckCircle2 } from "lucide-react";
...
export default function CreatePoll() {
  ...
  const [questions, setQuestions] = useState([
    { text: "", options: ["", ""], isMandatory: true, correctOptionIndex: null }
  ]);

  const addQuestion = () => {
    setQuestions([...questions, { text: "", options: ["", ""], isMandatory: true, correctOptionIndex: null }]);
  };
  ...
  const removeOption = (qIndex, oIndex) => {
    ...
    if (newQuestions[qIndex].correctOptionIndex === oIndex) newQuestions[qIndex].correctOptionIndex = null;
    setQuestions(newQuestions);
  };

  const toggleMandatory = (index) => {
    const newQuestions = [...questions];
    newQuestions[index].isMandatory = !newQuestions[index].isMandatory;
    setQuestions(newQuestions);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Basic Validation
      if (!pollData.title.trim()) throw new Error("Campaign title is required");
      ...
      <div className="mb-12 border-b border-white/5 pb-10">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          <div className="flex items-center gap-3 mb-2">
            ...
            <p className="text-[10px] uppercase tracking-[0.3em] text-white/30 font-bold">New Deployment</p>
          </div>
          <h1 className="font-display text-5xl font-normal tracking-[-0.05em] text-white md:text-7xl leading-tight">
            CAMPAIGN <br /><span className="text-white/40 italic"><Highlight>Architect</Highlight></span>
          </h1>
        </motion.div>
      </div>
             ...
             <div className="surface rounded-[40px] p-8 border border-white/5 bg-white/[0.01]">
                <div className="grid gap-8">
                  <div className="group relative">
                    <label className="text-[10px] uppercase tracking-[0.2em] text-white/30 font-bold mb-3 block">Campaign Identity</label>
                    <input 
                      type="text"
                      placeholder="Title of the operation..." 
                  ...
                  <div>
                    <label className="text-[10px] uppercase tracking-[0.2em] text-white/30 font-bold mb-3 block">Strategy Overview</label>
                    <textarea 
                      placeholder="Context for the participants..." 
              ...
              {questions.map((q, qIdx) => (
                ...
                >
                  <div className="absolute -left-4 top-10 flex flex-col gap-2">
                     <span className="h-8 w-8 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-[10px] font-bold text-[#ef4444] backdrop-blur-xl">{qIdx + 1}</span>
                  </div>
                  ...
                  <div className="space-y-8">
                    <div className="group relative">
                      <label className="text-[10px] uppercase tracking-[0.2em] text-white/30 font-bold mb-3 block">Question Text</label>
                      <input 
                        type="text"
                    ...
                    <div className="flex items-center justify-between border-t border-white/5 pt-6">
                       <div 
                        onClick={() => toggleMandatory(qIdx)}
                        className="flex items-center gap-3 cursor-pointer group"
                       >
                          <div className={`h-4 w-4 rounded-md border transition-all flex items-center justify-center ${q.isMandatory ? "bg-[#ef4444] border-[#ef4444]" : "border-white/10"}`}>
                             {q.isMandatory && <X size={12} className="text-white" />}
                          </div>
                          <span className={`text-[10px] font-bold uppercase tracking-widest ${q.isMandatory ? "text-white" : "text-white/20"}`}>Mandatory Requirement</span>
                       </div>
                       <span className="font-handwriting text-white/10 text-xl italic select-none">Vox Populi</span>
                    </div>
                  </div>
                </motion.div>
              ))}
          ...
          <div className="sticky top-8 space-y-8">
            ...
            >
              ...
              <h2 className="font-display text-xl text-white mb-8 flex items-center gap-3">
                 Protocols <span className="text-[9px] bg-white/10 px-2 py-0.5 rounded text-white/40 font-bold tracking-widest uppercase">Global</span>
              </h2>
              ...
              <div className="space-y-8">
                ...
                <div className="space-y-3">
                  <div className="flex items-center gap-2 text-[#ef4444]">
                    ...
                    <label className="text-[9px] uppercase tracking-[0.2em] font-black opacity-60">Visibility</label>
                  </div>
                  <select 
                  ...
                  >
                    <option value="public" className="bg-[#0a0a0a]">Public Access</option>
                    <option value="unlisted" className="bg-[#0a0a0a]">Unlisted Link</option>
                    <option value="private" className="bg-[#0a0a0a]">Private Vault</option>
                  </select>
                </div>
                ...
                <div className="space-y-3">
                  <div className="flex items-center gap-2 text-amber-400">
                    ...
                    <label className="text-[9px] uppercase tracking-[0.2em] font-black opacity-60">Deadline</label>
                  </div>
                  <input 
                    type="datetime-local" 
                ...
                {/* Anonymous Toggle */}
                <div 
                  onClick={() => setPollData({...pollData, settings: {...pollData.settings, anonymous: !pollData.settings.anonymous}})}
                ...
                >
                  <div className="flex items-center gap-3 relative z-10">
                    ...
                    <div>
                      <p className={`text-[12px] font-bold tracking-tight transition-colors ${pollData.settings.anonymous ? "text-white" : "text-white/40"}`}>Incognito</p>
                      <p className="text-[8px] text-white/20 uppercase font-bold tracking-widest">No auth required</p>
                    </div>
                  </div>
                ...
                >
                  {loading ? <Loader2 className="animate-spin" size={20} /> : (
                    <span className="flex items-center gap-3">
                      IGNITION <span className="h-[1px] w-4 bg-current opacity-30" /> LAUNCH
                    </span>
                  )}
                </button>
              </div>
