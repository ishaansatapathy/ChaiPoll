...
export default function PollView() {
  ...
  const handleSubmit = async () => {
    ...
    if (missing.length > 0) {
      setError(`Please answer: ${missing[0].text}`);
      return;
    }
    setSubmitting(true);
    setError("");
  ...
  if (error && !poll)
    return (
      <div className="min-h-screen bg-[#020202] flex flex-col items-center justify-center p-6 text-center">
        ...
        <h1 className="text-2xl font-display text-white mb-2">Poll Not Found</h1>
      ...
      <section className="max-w-3xl mx-auto px-6 py-32 relative z-10">
        ...
        {needsAuth ? (
          <motion.div
            ...
            className="surface rounded-[40px] p-12 border border-[#ef4444]/20 bg-[#ef4444]/[0.02] text-center"
          >
            <Shield className="text-[#ef4444] mx-auto mb-6" size={48} />
            <h2 className="text-2xl text-white font-display mb-4">Sign In Required</h2>
            ...
            {poll.questions.map((q, qIdx) => (
              ...
              >
                <div className="mb-6 flex items-baseline gap-4">
                  <span className="font-handwriting text-3xl text-[#ef4444] opacity-40">
                    0{qIdx + 1}
                  </span>
                  <h3 className="text-2xl text-white font-display flex items-center gap-3">
                    {q.text}
                    {q.isMandatory && (
                      <span className="text-[10px] text-[#ef4444] border border-[#ef4444]/30 px-2 py-0.5 rounded uppercase font-bold">
                        Required
                      </span>
                    )}
                  </h3>
            ...
            <div className="pt-12 border-t border-white/5">
              {error && (
                <motion.div
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  className="mb-8 p-4 rounded-2xl bg-red-500/10 border border-red-500/20 text-sm text-red-400 font-bold uppercase tracking-widest text-center flex items-center justify-center gap-2"
                >
              ...
              <div className="mt-8 text-center">
                <span className="font-handwriting text-white/10 text-2xl rotate-1 block italic">
