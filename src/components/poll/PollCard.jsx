...
export function PollCard({ poll, onDeleteSuccess }) {
  ...
  return (
    <>
      <Card className={`transition duration-500 hover:border-white/20 hover:bg-white/[0.04] ${deleting ? "opacity-50 grayscale pointer-events-none" : ""}`}>
        ...
        <div className="mt-6 pt-6 border-t border-white/5 flex flex-wrap items-center justify-between gap-4">
          <div className="flex gap-4 text-[10px] font-bold uppercase tracking-widest">
            <Link to={`/results/${poll.pollCode}`} className="text-white hover:text-[#ef4444] transition-colors">Results</Link>
            <Link to={`/poll/${poll.pollCode}`} className="text-white/40 hover:text-white transition-colors">Form Preview</Link>
          </div>
          {isOwner && (
            <Link 
